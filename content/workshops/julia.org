...
Started in 2009 by Jeff Bezanson, [[https://en.wikipedia.org/wiki/Stefan_Karpinski][Stefan Karpinski]], [[https://en.wikipedia.org/wiki/Viral_B._Shah][Viral B. Shah]], and [[https://en.wikipedia.org/wiki/Alan_Edelman][Alan Edelman]], Julia was launched in 2012 as free and open source software. Version 1.0 was released in 2018.
...
While we will be interacting with Julia through the REPL to develop and test our code, we want to save it in a script for future use and to submit to the cluster.
...
Open a new terminal window and from your own computer run:
...
*** Access Julia and install packages
...
So the normal process to get started with Julia in the cluster would be to run:
#+BEGIN_src sh
module load gcc/7.3.0 julia/1.3.0
#+END_src

We would then install the necessary Julia packages.

#+BEGIN_export html
In order to take a lot of burden out of our training cluster however, since we are all running the same code with the same packages, instead of having each of us install the packages in our home directories, we will all make use of packages which have already been installed in <span style="font-family: 'Source Code Pro', 'Lucida Console', monospace; font-size: 1.4rem; padding: 0.3rem; box-shadow: 0px 0px 2px rgba(0,0,0,0.3); border-radius: 5%; background-color: #f0f3f3; color: #000000">/project/shared/bio</span>.

So what we will do instead is run a little script called <span style="font-family: 'Source Code Pro', 'Lucida Console', monospace; font-size: 1.4rem; padding: 0.3rem; box-shadow: 0px 0px 2px rgba(0,0,0,0.3); border-radius: 5%; background-color: #f0f3f3; color: #000000">/project/shared/syncHPC/startJulia.sh</span>.
#+END_export
...
#+BEGIN_export html
As you can see, the Julia module is already loaded. The rest sets the path to the packages to <span style="font-family: 'Source Code Pro', 'Lucida Console', monospace; font-size: 1.4rem; padding: 0.3rem; box-shadow: 0px 0px 2px rgba(0,0,0,0.3); border-radius: 5%; background-color: #f0f3f3; color: #000000">/project/shared/bio</span>.
#+END_export

So, for the purpose of this workshop, instead of loading Julia and installing the required packages, we will simply run:
...
# module load gcc/7.3.0 julia/1.3.0
# but instead we are here running startJulia.sh
# which does this and changes the environment variables
# which tell Julia where to look for packages
source /project/shared/syncHPC/startJulia.sh
